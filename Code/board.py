...
class Board():
    ...
    def draw_ring(self, player_num):
        ...
        """
        # draw all tiles
        for tile_num in range(self.tiles_per_ring):
            self.draw_tile(tile_num, player_num)
        
    def draw_tile(self, tile_num, player_num):
        """ 
        ...
        pg.draw.arc(self.screen, self.black, [0.5*self.width-distance, 0.5*self.height-distance, distance*2, distance*2], tile_num*(2*math.pi/7), (tile_num*(2*math.pi/7))+2*math.pi/7, int(self.ring_width*0.5))   
